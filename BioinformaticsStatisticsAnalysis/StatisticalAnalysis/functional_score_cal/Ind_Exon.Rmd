...
for (i in 1:length(filenames)){
  ...
  #                                     trim_ws = TRUE)
 test1 <-read_tsv(paste0(dtdir, filenames[i]), num_threads = 1)
...
if(length(filenames) >4) { # five experiments
  ### calculate ratio
  wide <- wide %>% mutate(R5_lib_freq = log2(R5_lib/sum(R5_lib, na.rm = T)),
                          R5_D5_freq = log2(R5_D5/sum(R5_D5, na.rm = T)),
                          R5_D14_freq = log2(R5_D14/sum(R5_D14, na.rm = T)),
                          
                          R5_D5_lib = R5_D5_freq - R5_lib_freq,
                          R5_D14_lib= R5_D14_freq - R5_lib_freq,
                          R5_D14_D5 = R5_D14_freq - R5_D5_freq
  )%>% arrange(POS)
}
if(length(filenames) >5) { # six experiments
  ### calculate ratio
  wide <- wide %>% mutate(R6_lib_freq = log2(R6_lib/sum(R6_lib, na.rm = T)),
                          R6_D5_freq = log2(R6_D5/sum(R6_D5, na.rm = T)),
                          R6_D14_freq = log2(R6_D14/sum(R6_D14, na.rm = T)),
                          
                          R6_D5_lib = R6_D5_freq - R6_lib_freq,
                          R6_D14_lib= R6_D14_freq - R6_lib_freq,
                          R6_D14_D5 = R6_D14_freq - R6_D5_freq
  )%>% arrange(POS)
}
```
...
Applying loess, plot D5vLib using variants used for loess fit, D5vLib with loess fit, D14vLib with loess; plot D14vD5 using variants used for loess fit, D14vD5 with loess.

```{r loess, include=TRUE, fig.width=10, fig.height= 12*length(filenames) }
...
After subsetting, replicate1 has `r lv.n1[1]` variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D5 values for each variant. 


`r ifelse(length(filenames) >1, paste0("After subsetting , replicate2 has ", lv.n1[2], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D5 values for each variant."), "") `


`r ifelse(length(filenames) >2, paste0("After subsetting , replicate3 has ", lv.n1[3], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D5 values for each variant."), "") `


`r ifelse(length(filenames) >3, paste0("After subsetting , replicate4 has ", lv.n1[4], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D5 values for each variant."), "") `

`r ifelse(length(filenames) >4, paste0("After subsetting , replicate4 has ", lv.n1[5], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D5 values for each variant."), "") `

D14vD5:

After subsetting, replicate1 has `r lv.n2[1]` variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D14vD5 values for each variant. 


`r ifelse(length(filenames) >1, paste0("After subsetting , replicate2 has ", lv.n2[2], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D14vD5 values for each variant."), "") `


`r ifelse(length(filenames) >2, paste0("After subsetting , replicate3 has ", lv.n2[3], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D14vD5 values for each variant."), "") `


`r ifelse(length(filenames) >3, paste0("After subsetting , replicate4 has ", lv.n2[4], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D14vD5 values for each variant."), "") `

`r ifelse(length(filenames) >4, paste0("After subsetting , replicate5 has ", lv.n2[4], " variants used for LOESS fitting then applied back to all the position in the previous data to get fitted D14vD5 values for each variant."), "") `
...
```{r adjustment, include=TRUE, fig.width=10, fig.height= 8*length(filenames) }
...
```{r adjustment2, include=TRUE, fig.width=10, fig.height= 8*length(filenames) }
...
```{r adjustment3, include=TRUE, fig.width=10, fig.height= 8*length(filenames) }
...
for (i in 1:length(filenames)){
  ...
  temp1i <- wide %>% mutate(functional.score = !!sym(D14_lib_ratio_adjusted_Ri.1)) %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") ) %>% arrange(functional.score) 
  temp2i <- wide %>% mutate(functional.score = !!sym(D14_D5_ratio_adjusted_Ri.1)) %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") )%>% arrange(functional.score)
...
for (i in 1:length(filenames)){
  ...
  temp1i <- wide %>% mutate(functional.score = !!sym(D14_lib_ratio_adjusted_Ri.1)) %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(spliceR == FALSE) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") ) %>% arrange(functional.score) 
  temp2i <- wide %>% mutate(functional.score = !!sym(D14_D5_ratio_adjusted_Ri.1)) %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(spliceR == FALSE) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") )%>% arrange(functional.score)
 temp.1 <- rbind(temp.1, temp1i)
 temp.2 <- rbind(temp.2, temp2i)
...
##
myroc <- function(data, scorecolumn, typecolumn, casen, controln){
  data_long <- data %>% filter(spliceR == FALSE) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>%filter( !!sym(typecolumn)%in% c(casen, controln))  %>% mutate(functional.score = !!sym(scorecolumn)) %>% filter(uPOS %in% wide2$uPOS)  %>% arrange(functional.score)
  ## plot
  par(mfrow = c(1, 1))
...
for (i in 1:length(filenames)){
  ...
  temp <- wide %>% filter(spliceR == FALSE) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") )%>% filter(EventType %in% c("Synonymous", "StopGain")) %>% mutate(functional.score = !!sym(D14_lib_ratio_adjusted_Ri.1)) %>% filter(uPOS %in% wide2$uPOS)
  data_long <- rbind(data_long, temp)
}
ROC0 <- myroc(data_long, scorecolumn = "functional.score", typecolumn = "EventType", casen ="StopGain" , controln = "Synonymous")
...
for (i in 1:length(filenames)){
  ...
  temp <- wide %>% filter(spliceR == FALSE) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G") )%>% filter(EventType %in% c("Synonymous", "StopGain")) %>% mutate(functional.score = !!sym(D14_D5_ratio_adjusted_Ri.1)) %>% filter(uPOS %in% wide2$uPOS)
  data_long <- rbind(data_long, temp)
}

ROC0.1 <- myroc(data_long, scorecolumn = "functional.score", typecolumn = "EventType", casen ="StopGain" , controln = "Synonymous")
...
mmfun <- function(data, scorecolumn, typecolumn, casen, controln){
  ...
  temp <- data %>% filter(!!sym(typecolumn) %in% c(casen, controln)) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>% rename(functional.score = !!sym(scorecolumn))%>% filter(!is.na(functional.score)) %>% filter(spliceR == FALSE) %>% filter(uPOS %in% wide2$uPOS)
...
p2 <- final2 %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(spliceR == FALSE) %>%filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>% arrange(functional.scoreA) %>%
  ggplot( aes(x=functional.scoreA, fill=EventType)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity',binwidth = 0.2)  +
...
p1 <- final1 %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>%filter(spliceR == FALSE) %>% arrange(functional.scoreC) %>% 
  ggplot( aes(x=functional.scoreC, fill=EventType)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity',binwidth = 0.2) + 
...
p1 <- final1 %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>%filter(spliceR == FALSE) %>% arrange(functional.scoreD) %>% 
  ggplot( aes(x=functional.scoreD, fill=EventType)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity',binwidth = 0.2) + 
  scale_fill_manual(values=c( "red", "#69b3a2")) +  ggtitle ( "D14vsLib functional scoreC")


p2 <- final2 %>% filter(EventType %in% c("StopGain", "Synonymous")) %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G")) %>%filter(spliceR == FALSE) %>% arrange(functional.scoreD) %>% 
  ggplot( aes(x=functional.scoreD, fill=EventType)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity',binwidth = 0.2)  +
  scale_fill_manual(values=c( "red", "#69b3a2")) +  ggtitle ( "D14_D5 functional scoreC")
...
for(score in c("A", "B", "C", "D")){
  ...
  ROCs0A <- myroc(final1, scorecolumn = paste0("functional.score",score), typecolumn = "EventType", casen = "StopGain", controln = "Synonymous")
...
for(score in c("A", "B", "C", "D")){
  ...
  ROCs0A.1 <- myroc(final2, scorecolumn = paste0("functional.score",score), typecolumn = "EventType", casen = "StopGain", controln = "Synonymous")
  cat("\n")
      cat("\n")
    print(ROCs0A.1$tb)
    cat("\n")
...
if(!is.null(params$HDRfile)){
  ...
  if(length(unique(dt$htype))>1){
    for(score in c("A", "B", "C", "D")){
      ...
      if(Na >0 & Nn > 0){
        ROC0hA <- myroc(dt, scorecolumn = paste0("functional.score",score), typecolumn = "htype", casen = "Abnormal", controln = "Normal")
      cat("\n")
      print(ROC0hA$tb)
      cat("\n")
      cat(ROC0hA$cutoff, " was used for the performance. sensitivity is ", ROC0hA$sen, " specificity is ", ROC0hA$spec, ".\n\n",
         ...
         ROC0hA <- list(n=NA, cutoff = NA, auc = NA, sen = NA, spec = NA, tb = NA)
      }
  ...
  if(length(unique(dt2$htype))>1){
    for(score in c("A", "B", "C", "D")){
      ...
      Na <- sum(!is.na(temp[temp$htype == "Abnormal",2])); Nn <- sum(!is.na(temp[temp$htype == "Normal",2]))
      if(Na >0 & Nn > 0){
        ROC0hA.1 <- myroc(dt2, scorecolumn = paste0("functional.score",score), typecolumn = "htype", casen = "Abnormal", controln = "Normal")
      cat("\n")
      print(ROC0hA.1$tb)
      cat("\n")
      cat(ROC0hA.1$cutoff, " was used for the performance. sensitivity is ", ROC0hA.1$sen, " specificity is ", ROC0hA.1$spec, ".\n\n",
        ...
        ROC0hA.1 <- list(n=NA, cutoff = NA, auc = NA, sen = NA, spec = NA, tb = NA)
      }
...
## 5 scores
final1.snv <- final1 %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G"))
final2.snv <- final2 %>% filter(REF %in% c("A", "T", "C", "G") & ALT %in% c("A", "T", "C", "G"))
if(!is.null(params$HDRfile)){
  if(length(unique(dt$htype))>1){
    ...
    mihdr = c(NA, sum(ROC0h$tbA) -  ifelse(is.na(sum(ROC0h$tbA)), NA, sum(diag(ROC0h$tbA))),
              sum(ROC0h$tbB) - ifelse(is.na(sum(ROC0h$tbB)), NA, sum(diag(ROC0h$tbB))), 
              sum(ROC0h$tbC) - ifelse(is.na(sum(ROC0h$tbC)), NA, sum(diag(ROC0h$tbC))), 
              sum(ROC0h$tbD) - ifelse(is.na(sum(ROC0h$tbD)), NA, sum(diag(ROC0h$tbD))))
...
metrics <- data.frame(Exon = rep(Exn,5),
                      ...
                      miscall=c(sum(ROC0$tb) - sum(diag(ROC0$tb)), sum(ROCs0$tbA) - sum(diag(ROCs0$tbA)), sum(ROCs0$tbB) - sum(diag(ROCs0$tbB)), sum(ROCs0$tbC) - sum(diag(ROCs0$tbC)), sum(ROCs0$tbD) - sum(diag(ROCs0$tbD))),
                      confusion.matrix = c(paste(as.vector((c(ROC0$tb[1,], ROC0$tb[2,]))), collapse = " "),
...
if(!is.null(params$HDRfile)){
  if(length(unique(dt2$htype))>1){
    ...
    mihdr = c(NA, sum(ROC0h.1$tbA) -  ifelse(is.na(sum(ROC0h.1$tbA)), NA,sum(diag(ROC0h.1$tbA))),
              sum(ROC0h.1$tbB) - ifelse(is.na(sum(ROC0h.1$tbB)), NA, sum(diag(ROC0h.1$tbB))), 
              sum(ROC0h.1$tbC) - ifelse(is.na(sum(ROC0h.1$tbC)), NA, sum(diag(ROC0h.1$tbC))), 
              sum(ROC0h.1$tbD) - ifelse(is.na(sum(ROC0h.1$tbD)), NA, sum(diag(ROC0h.1$tbD))))
...
metrics <- data.frame(Exon = rep(Exn,5),
                      ...
                      miscall= c(sum(ROC0.1$tb) - sum(diag(ROC0.1$tb)), sum(ROCs0.1$tbA) - sum(diag(ROCs0.1$tbA)), sum(ROCs0.1$tbB) - sum(diag(ROCs0.1$tbB)), sum(ROCs0.1$tbC) - sum(diag(ROCs0.1$tbC)), sum(ROCs0.1$tbD) - sum(diag(ROCs0.1$tbD))),
